...
import { planExecutionApi, type PlanExecution, type CreatePlanExecutionParams } from '@/api/planExecution'
...
const deleteTask = async (taskId: string) => {
  try {
    await planExecutionApi.delete(parseInt(taskId))
    toast.success('任务已删除')
    await loadTodayTasks()
  } catch (error) {
  ...
  <Dialog v-model:open="showEditTaskDialog">
    <DialogContent class="sm:max-w-[500px]">
      ...
      <div v-if="editingTask" class="space-y-6 py-4">
        ...
        <div class="space-y-2">
          <label class="text-[12px] font-bold uppercase tracking-widest text-neutral-400 ml-1">优先级</label>
          <Select v-model="editingTask.priority" disabled>
            <SelectTrigger class="w-full px-5 py-3.5 bg-stone-50 border border-black/5 rounded-2xl text-sm">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
